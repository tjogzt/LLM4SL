...
\subsection{Orthogonal Co-Dependency Evidence from DepMap}
...
\subsection{ESM-2 Gene-Specific Protein Sequence Contribution}
...
ESM-2 M×50 placeholder ($\sim$250 genes) & 0.624 $\pm$ 0.032 \\
No ESM-2 (text features only) & 0.708 $\pm$ 0.054 \\
\bottomrule
\end{tabular}
\end{table}

Table.*shows the contribution of Table~\ref{tab:esm}. Real UniProt canonical sequences (15,869/15,956 genes, 99.5% coverage) provide a +9.0% AUC gain, while M×50 placeholder sequences actually degrade performance by -8.4%, confirming that amino acid identity matters. The reason, we suspect, is that ESM-2's pre-training on UniRef has baked in enough fundamental chemistry (hydrophobicity scales, charge patterns, secondary structure propensities) that even an artificial sequence triggers useful representations. But when only $\sim$250 genes out of 16,000 have real sequences, the remaining 15,700+ embeddings are zero vectors. That sparsity creates a sharp feature discontinuity that hurts more than it helps. We call this the ``gene-specific protein sequence signal'' and return to its implications in the Discussion.
...
In accordance with \textit{Briefings in Bioinformatics} policy on generative AI, we disclose the following. DeepSeek-V4-Pro (DeepSeek, Beijing) was used via API as the named entity recognition engine for literature mining, as described in Methods~\S2.2; no LLM-generated text was incorporated into the manuscript without human review. ESM-2 embeddings were computed using Meta's publicly released model weights (\texttt{esm2\_t12\_35M\_UR50D}) with standard inference. Language editing assistance was provided by an LLM-based tool in select passages; all such edits were reviewed and revised by the author. No LLM or AI tool is listed as an author, and the author accepts full responsibility for all content.
...
\begin{figure}[p]
    ...
    \caption{Method overview. The framework combines LLM-driven literature mining for gene co-mention weak supervision, GATv2 message passing on the STRING PPI network, and ESM-2 embeddings from real UniProt sequences (15,869/15,956 genes).for sequence-level priors.}
    \label{fig:method}
\end{figure}
...
\begin{figure}[p]
    ...
    \caption{Incremental contribution of information channels. Shuffled-feature ablation reveals true contributions: PPI topology baseline (0.790) $\rightarrow$ +LLM text (+1.6\%, 0.803) $\rightarrow$ +ESM-2 embeddings (+9.0\%, 0.798). Zero-vector ablation is shown for comparison to illustrate the 14$\times$ inflation artifact.}
...
Targeting the DNA repair defect in BRCA mutant cells in preclinical investigation.
